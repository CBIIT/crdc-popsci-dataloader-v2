...
def process_arguments(args, log):
    ...
    if config.s3_folder:
        ...
        if response["KeyCount"] > 0:
            file_list = [f"s3://{config.s3_bucket}/" + i["Key"] for i in response["Contents"] if i["Key"][-3:] != "log"]
...
def main():
    ...
    try:
        ...
        if len(file_list) == 0:  # if s3 bucket was supplied, file list comes from S3 and ignores local
        ...
        if not config.s3_bucket:
            log.info("file list will be loaded the local path provided in config file")
